...
test.describe("Account Management", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="add-account-button"]', {
      timeout: 30000,
    });
  });
  ...
  test("should display General Ledger page", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="ledger-title"]', {
      timeout: 20000,
    });
  ...
  test.describe("Chart of Accounts", () => {
    ...
    test("should validate required fields when creating account", async ({
      ...
      await page.waitForSelector('[data-testid="add-account-modal"]', {
        timeout: 10000,
      });
    ...
    test("should handle API errors gracefully", async ({ page }) => {
      ...
      await expect(
        page
          .locator('[data-testid="add-account-modal"]')
          .locator("text=Account code already exists")
      ).toBeVisible();
    });
  ...
  test.describe("General Ledger", () => {
    ...
    test("should successfully create new account", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="account-name-input"]')
        .fill("Test Liability Account");

      // Select account type
      await page.locator('[data-testid="account-type-select"]').click();
      await page
        .locator('[data-testid="account-type-option-LIABILITY"]')
        .click();

      // Try to select normal balance if available
      const normalBalanceSelect = page.locator(
      ...
      if (await normalBalanceSelect.isVisible()) {
        ...
        const creditOption = page.locator(
          '[data-testid="normal-balance-option-credit"]'
        );
        if (await creditOption.isVisible()) {
          await creditOption.click();
        } else {
          console.log(
            "Normal balance selection skipped: Credit option not found"
          );
        }
      } else {
    ...
    test("should handle API errors when creating accounts", async ({
      page,
    }) => {
      // Click Add Account button
      await page.locator('[data-testid="add-account-button"]').click();
  ...
  test.describe("Edge Cases", () => {
    test("should handle special characters in account names", async ({
      page,
    }) => {
      // Click Add Account button
      await page.locator('[data-testid="add-account-button"]').click();
    ...
    test("should handle very long account names", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="account-name-input"]')
        .fill(
          "This is a very long account name that should be handled properly by the system without causing any issues or breaking the UI layout"
        );
      ...
      await expect(
        page.locator('[data-testid="add-account-modal"]')
      ).toBeVisible();
    });

    test("should handle concurrent account creation attempts", async ({
      page,
    }) => {
      // Click Add Account button
      await page.locator('[data-testid="add-account-button"]').click();
