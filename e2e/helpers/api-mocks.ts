...
export class E2EApiMocker {
              ...
              insights: [
                {
                  ...
                  description:
                    "AI analysis suggests potential for 15% revenue growth",
                  confidence: 0.85,
                  priority: "medium",
                },
              ],
   ...
   */
  addMockAccount(
    account: Omit<MockAccount, "id" | "createdAt" | "updatedAt">
  ): MockAccount {
    const newAccount: MockAccount = {
      ...account,
      id: this.nextAccountId.toString(),
 ...
 */
export async function setupGlobalApiMocks(
  page: any,
  preserveAuth: boolean = true
): Promise<E2EApiMocker> {
  // Changed from Page to any
  ...
  await page.route("**/api/**", async (route: Route) => {
    ...
    if (url.includes("/api/auth/login") && method === "POST") {
      ...
      if (
        ...
        (email?.match(/^test\d+@example\.com$/) &&
    ...
    if (url.includes("/api/auth/profile") && method === "GET") {
      ...
      if (
        ...
        authHeader.includes("mock-jwt-token")
      ) {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    if (url.includes("/api/accounts") && method === "POST") {
      ...
      if (
        accountData.code === "1003" &&
        accountData.name === "Duplicate Account"
      ) {
        await route.fulfill({
          status: 400,
          contentType: "application/json",
          ...
          result: {
            insights: [
              ...
              },
              {
                ...
                description:
                  "Operating expenses have increased by 8% this quarter",
                confidence: 0.92,
                priority: "high",
              },
            ],
