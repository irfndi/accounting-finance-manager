...
test.describe("Simple Login Test", () => {
  test("should render login form and submit via JavaScript", async ({
    page,
  }) => {
    // Setup API mocks
    ...
    page.on("response", (response) => {
      if (
        !response.ok() &&
        (response.url().includes("react") || response.url().includes("astro"))
      ) {
        failedRequests.push({
          url: response.url(),
    ...
    console.log(
      "API requests:",
      requests.filter((r) => r.includes("/api/"))
    );
    ...
    const hasReactScript =
      pageContent.includes("react") || pageContent.includes("React");
    ...
    console.log("Page analysis:", {
      ...
      astroIslandHTML:
        pageContent.match(/<astro-island[^>]*>/)?.[0] || "Not found",
    });
    ...
    const postWaitReact = await page.evaluate(() => {
      return {
        ...
        formHasEventListeners:
          document.querySelector("form")?._reactInternalFiber !== undefined ||
          document.querySelector("form")?._reactInternalInstance !==
            undefined ||
