...
setup("authenticate", async ({ page }) => {
  ...
  while (retries > 0) {
    try {
      ...
      if (retries === 0) {
        console.error(
          "❌ Failed to navigate to login page after retries:",
          error
        );
        throw error;
      }
      console.log(
        `⚠️ Navigation failed, retrying... (${retries} attempts left)`
      );
  ...
  for (let i = 0; i < 3; i++) {
    ...
    console.log(
      `📝 Retry ${
        i + 1
      }: Email value: "${emailValue}", Password value: "${passwordValue}"`
    );
  ...
  console.log(
    `📝 Final values - Email: "${emailValue}", Password: "${passwordValue}"`
  );

  if (!emailValue || !passwordValue) {
    throw new Error(
      `Form fields not filled properly after retries. Email: "${emailValue}", Password: "${passwordValue}"`
    );
  }
