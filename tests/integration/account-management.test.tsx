...
describe("ChartOfAccounts Integration Tests", () => {
  ...
  describe("Account List Display", () => {
    it("should load and display accounts on mount", async () => {
      ...
      mockFetch = setupMockFetch({
        "GET:/api/accounts": { accounts: mockAccounts },
        "GET:/api/account-types": {
          types: mockAccountTypes,
        },
        "GET:/api/currencies": {
    ...
    it("should display account details correctly", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("Loading accounts..."),
  ...
  describe("Account Creation", () => {
    ...
    it.skip("should create a new account successfully", async () => {
      ...
      mockFetch = setupMockFetch({
        ...
        "POST:/api/accounts": { account: newAccount },
      });
      ...
      await waitFor(() => {
        ...
        expect(requestBody.balance).toBe(1000.0);
  ...
  describe("Account Editing", () => {
    it("should open edit form when Edit button is clicked", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Edit Account: Cash Account"),
        ).toBeInTheDocument();
        expect(screen.getByDisplayValue("Cash Account")).toBeInTheDocument();
        expect(
          screen.getByDisplayValue("Primary cash account for daily operations"),
        ).toBeInTheDocument();
      });
    });
    ...
    it.skip("should update account successfully", async () => {
      ...
      mockFetch = setupMockFetch({
        ...
        "PUT:/api/accounts/1": { account: updatedAccount },
      });
      ...
      const descriptionInput = screen.getByDisplayValue(
        "Primary cash account for daily operations",
      );
  ...
  describe("Account Deactivation/Activation", () => {
    it.skip("should deactivate an account", async () => {
      ...
      mockFetch = setupMockFetch({
        ...
        "PATCH:/api/accounts/1/deactivate": {
      ...
      await waitFor(() => {
        ...
        expect(patchCall![0]).toBe("/api/accounts/1/deactivate");
      });
    });
    ...
    it.skip("should show inactive accounts when toggle is enabled", async () => {
      ...
      const showInactiveToggle = screen.getByLabelText(
        /show inactive accounts/i,
      );
  ...
  describe("Account Filtering and Search", () => {
    it.skip("should filter accounts by type", async () => {
      ...
      console.log(
        "Available comboboxes:",
        screen.getAllByRole("combobox").map((el) => el.outerHTML),
      );
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText("Savings Account")).toBeInTheDocument();
        expect(
          screen.queryByText("Credit Card - Business"),
        ).not.toBeInTheDocument();
      });
    });
    ...
    it("should search accounts by name", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Cash Account")).toBeInTheDocument();
        expect(
          screen.queryByText("Bank Account - Checking"),
        ).not.toBeInTheDocument();
        expect(
          screen.queryByText("Credit Card - Business"),
        ).not.toBeInTheDocument();
  ...
  describe("Error Handling", () => {
    it("should handle API errors gracefully", async () => {
      ...
      mockFetch = setupMockFetch(
        {
          "GET:/api/accounts": null, // This will cause fetch to reject
        },
        true,
      );
    ...
    it.skip("should show validation errors for empty form submission", async () => {
      mockFetch = setupMockFetch({
        ...
        "POST:/api/accounts": null, // This should not be called due to validation
      });
      ...
      await waitFor(
        () => {
          expect(
            screen.getByText("Account code is required"),
          ).toBeInTheDocument();
        },
        { timeout: 3000 },
      );
            ...
            return Promise.resolve({
              ...
              json: () =>
                Promise.resolve({
                  ...
                  error:
                    "Account Cash Account has 5 existing transactions and cannot be deleted",
    ...
    it("should handle account data export", async () => {
      ...
      await waitFor(() => {
        const exportCall = mockFetch.mock.calls.find(
          (call) => call[0] === "/api/accounts/export",
        );
