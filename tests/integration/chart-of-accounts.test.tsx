...
vi.mock("../../src/web/components/ui/alert-dialog", () => ({
  ...
  AlertDialogTrigger: ({ children, onClick }: any) => (
    <button onClick={onClick} data-testid="alert-dialog-trigger">
      {children}
    </button>
  ),
}));
    ...
    async () => {
      try {
        ...
        try {
          ...
          console.log(
            `Could not interact with type selector, proceeding with default type: ${type}`,
          );
        }
      }
    },
...
describe("ChartOfAccounts Component", () => {
  ...
  it("should validate required fields before submission", async () => {
    ...
    await waitFor(
      () => {
        ...
        const formStillPresent = screen.queryByRole("button", {
          name: /create/i,
        });
  ...
  it("should handle API errors gracefully", async () => {
    ...
    await waitFor(
      () => {
        ...
        const errorMessage =
          screen.queryByText("Account code already exists") ||
  ...
  it("should filter accounts by type", async () => {
    ...
    const typeFilter =
      screen.getByRole("combobox") || screen.getByDisplayValue("All Types");
    ...
    mockFetch = setupMockFetch(
      {
        ...
        "DELETE:/api/accounts/1": { success: true },
      },
      mockAccounts,
    );
