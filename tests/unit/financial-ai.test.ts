...
describe.skip("FinancialAIService", () => {
  ...
  describe("analyzeTransaction", () => {
    it("should analyze a transaction successfully", async () => {
      ...
      const result = await financialAIService.analyzeTransaction(
        mockTransaction
      );
    ...
    it("should throw an error if AI service fails", async () => {
      vi.mocked(mockAIService.generateText).mockRejectedValue(
        new Error("AI service unavailable")
      );

      await expect(
        financialAIService.analyzeTransaction(mockTransaction)
  ...
  describe("categorizeExpense", () => {
    it("should categorize an expense successfully", async () => {
      ...
      const result = await financialAIService.categorizeExpense(
        "New stapler",
        20.5
      );
    ...
    it("should throw an error if AI service fails during categorization", async () => {
      ...
      await expect(
        financialAIService.categorizeExpense("Test", 10)
    ...
    it("should extract structured data from OCR result", async () => {
      const mockExtractionResponse = {
        content: JSON.stringify({
          ...
          items: [{ description: "Office supplies", amount: 150.5 }],
        }),
      ...
      const result = await financialAIService.extractDocumentData(
        mockOCRResult,
        "invoice"
      );
