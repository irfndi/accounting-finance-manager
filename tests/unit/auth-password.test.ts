...
describe("Password Authentication", () => {
  ...
  beforeEach(() => {
    ...
    (global.crypto.subtle.deriveBits as any).mockResolvedValue(
      mockArrayBuffer.slice()
    );
  ...
  describe("hashPassword", () => {
    ...
    it("should generate different hashes for the same password", async () => {
      ...
      let callCount = 0;
      (global.crypto.getRandomValues as any).mockImplementation(
        (array: Uint8Array) => {
          for (let i = 0; i < array.length; i++) {
  ...
  describe("validatePassword", () => {
    ...
    it("should reject password with only letters", () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        "Password should contain numbers or special characters"
      );
    });
