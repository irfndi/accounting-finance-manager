    ...
    for (const entry of entries) {
      ...
      if (stats.isDirectory()) {
        ...
        if (
          ...
          !exclude.some((ex) => fullPath.includes(ex))
        ) {
          files.push(fullPath);
        }
      }
...
async function analyzeFile(filePath) {
  try {
    ...
    const classPattern =
      /(?:export\s+)?class\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*(?:extends\s+[a-zA-Z_$][a-zA-Z0-9_$]*)?\s*\{/g;
...
async function analyzeCoverage() {
  ...
  // Scan source files
  const sourceFiles = await scanDirectory(
    srcDir,
    [".ts", ".tsx"],
  ...
  const totalSourceFunctions = sourceAnalysis.reduce(
    (sum, file) => sum + file.functions.length,
    0
  );
  const totalSourceClasses = sourceAnalysis.reduce(
    (sum, file) => sum + file.classes.length,
    0
  );
  const totalSourceLines = sourceAnalysis.reduce(
    (sum, file) => sum + file.totalLines,
    0
  );

  const totalTestFunctions = testAnalysis.reduce(
    (sum, file) => sum + file.functions.length,
    0
  );
  const totalTestLines = testAnalysis.reduce(
    (sum, file) => sum + file.totalLines,
    0
  );
  ...
  console.log("-".repeat(40));
  console.log(
    `Function Coverage Ratio: ${(functionCoverageRatio * 100).toFixed(1)}%`
  );
  ...
  for (const sourceFile of sourceAnalysis) {
    ...
    const hasTest = testAnalysis.some(
      (test) =>
        test.file.includes(testFileName) ||
        test.file.includes(
          sourceFile.file.replace("src/", "").replace(/\.(ts|tsx)$/, "")
        )
    );
  ...
  filesWithoutTests.slice(0, 10).forEach((file) => {
    console.log(
      `📄 ${file.file} (${file.functions.length} functions, ${file.classes.length} classes)`
    );
  });
  ...
  if (functionCoverageRatio < 0.8) {
    const neededTests = Math.ceil(
      totalSourceFunctions * 0.8 - totalTestFunctions
    );
