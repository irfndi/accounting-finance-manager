...
describe("AIService", () => {
  ...
  const mockMessages: AIMessage[] = [{ role: "user", content: "Test message" }];
  ...
  describe("generateText", () => {
    ...
    it("should fallback to secondary provider when primary fails", async () => {
      ...
      vi.mocked(mockPrimaryProvider.generateText).mockRejectedValue(
    ...
    it("should throw AIServiceError when all providers fail", async () => {
      ...
      vi.mocked(mockFallbackProvider.generateText).mockRejectedValue(
        new Error("Fallback failed")
      );

      await expect(aiService.generateText(mockMessages)).rejects.toThrow(
        AIServiceError
      );
      await expect(aiService.generateText(mockMessages)).rejects.toThrow(
    ...
    it("should not retry on rate limit errors", async () => {
      const rateLimitError = new AIRateLimitError(
        "Rate limited",
        "test-provider",
        60
      );
    ...
    it("should handle timeout correctly", async () => {
      ...
      await expect(
        shortTimeoutService.generateText(mockMessages)
    ...
    it("should pass options to provider", async () => {
      ...
      expect(mockPrimaryProvider.generateText).toHaveBeenCalledWith(
        mockMessages,
        options
      );
    });
  ...
  describe("generateStream", () => {
    ...
    it("should stream from primary provider when available", async () => {
      vi.mocked(mockPrimaryProvider.generateStream!).mockImplementation(
        async function* () {
      ...
      expect(mockPrimaryProvider.generateStream).toHaveBeenCalledWith(
        mockMessages,
        undefined
      );
    });
    ...
    it("should fallback to secondary provider for stream when primary fails", async () => {
      ...
      vi.mocked(mockFallbackProvider.generateStream!).mockImplementation(
        async function* () {
          yield mockStreamChunk;
          yield mockFinalChunk;
        }
      ...
      expect(mockFallbackProvider.generateStream).toHaveBeenCalledWith(
        mockMessages,
        undefined
      );
    });
  ...
  describe("getProvidersHealth", () => {
    ...
    it("should handle provider availability errors", async () => {
      vi.mocked(mockPrimaryProvider.isAvailable).mockRejectedValue(
...
describe("createAIService", () => {
  ...
  beforeEach(async () => {
    ...
    (getAPIKey as Mock).mockImplementation(
