  ...
  describe("MagicLinkManager", () => {
    ...
    describe("generateMagicLink", () => {
      ...
      it("should generate different tokens for same email", async () => {
        const result1 = await magicLinkManager.generateMagicLink(
          testEmail,
          testPurpose
        );
        const result2 = await magicLinkManager.generateMagicLink(
          testEmail,
          testPurpose
        );
      ...
      it("should store metadata when provided", async () => {
        ...
        await magicLinkManager.generateMagicLink(testEmail, testPurpose, {
          metadata,
        });
    ...
    describe("verifyMagicLink", () => {
      it("should verify a valid magic link", async () => {
        ...
        // Verify it
        const result = await magicLinkManager.verifyMagicLink(
          token,
          testPurpose
        );
    ...
    describe("generateMagicLinkUrl", () => {
      it("should generate correct URL for login purpose", () => {
        ...
        expect(url).toBe(
          "https://example.com/auth/magic-login?token=test-token"
        );
      });
      ...
      it("should generate correct URL for register purpose", () => {
        ...
        expect(url).toBe(
          "https://example.com/auth/magic-register?token=test-token"
        );
      });
      ...
      it("should include redirect parameter when provided", () => {
        ...
        expect(url).toBe(
          "https://example.com/auth/magic-login?token=test-token&redirect=%2Fdashboard"
        );
      });
      ...
      it("should handle all purpose types", () => {
        ...
        const purposes = [
          ...
          {
            purpose: MagicLinkPurpose.RESET_PASSWORD,
            path: "/auth/reset-password",
          },
          {
            purpose: MagicLinkPurpose.CHANGE_EMAIL,
    ...
    describe("generateEmail", () => {
      ...
      it("should generate register email template", () => {
        ...
        expect(email.subject).toContain(
          "Complete your Finance Corp registration"
        );
