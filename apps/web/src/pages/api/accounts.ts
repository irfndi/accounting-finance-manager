...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (!db) {
      console.error(
        "Database not available. This API requires Cloudflare D1 database."
      );
      console.error("Current adapter: Node.js (expected: Cloudflare)");
      console.error(
        "To fix: Switch to Cloudflare adapter or set up local database"
      );
      ...
      return new Response(
        JSON.stringify({
          ...
          solution:
            "Switch to Cloudflare adapter in astro.config.mjs or configure local database",
          debug: {
            adapter: "node",
    ...
    const { dbAdapter: _dbAdapter, accountRegistry } =
      await createAccountingServices(db, entityId);
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (codeError) {
      return new Response(JSON.stringify({ error: codeError }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (typeError) {
      return new Response(JSON.stringify({ error: typeError }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    const existingAccountQuery = db.prepare(
      "SELECT id FROM accounts WHERE code = ? AND entity_id = ?"
    );
