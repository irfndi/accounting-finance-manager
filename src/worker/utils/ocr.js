 ...
 */
export function validateOCRRequirements(mimeType, fileSize, fileName, logger) {
    ...
    try {
        ...
        const maxSize = OCR_FILE_SIZE_LIMITS[mimeType];
 ...
 */
function classifyOCRError(error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    if (error instanceof ValidationError) {
        return {
            code: error.code || OCR_ERROR_CODES.VALIDATION_FAILED,
 ...
 */
function updateOCRMetrics(result, errorCode) {
 ...
 */
async function processFallbackOCR(fileData, mimeType, _options, logger, fileId) {
...
export async function processOCR(ai, fileData, mimeType, options = {}, fileId, fileName) {
    ...
    try {
        ...
        if (aiResponse && typeof aiResponse === 'object') {
            ...
            if (aiResponse.text) {
                extractedText = String(aiResponse.text);
            }
            else if (aiResponse.result && aiResponse.result.text) {
                extractedText = String(aiResponse.result.text);
            }
            ...
            if (options.includeConfidence) {
                confidence = aiResponse.confidence || aiResponse.result?.confidence || 0;
        ...
        if (!extractedText || extractedText.trim().length === 0) {
            logger.processingFailure(fileId || 'unknown', 'No text could be extracted from the document', processingTime);
            return {
                success: false,
        ...
        logger.processingSuccess(fileId || 'unknown', cleanedText.length, confidence, processingTime);
 ...
 */
export async function processFileOCR(ai, r2Bucket, fileId, options = {}) {
    const logger = createOCRLogger({ fileId });
    try {
        // Get file from R2
        const storageResult = await withOCRErrorBoundary('R2 File Retrieval', async () => {
 ...
 */
export async function batchProcessOCR(ai, files, options = {}) {
    const logger = createOCRLogger();
    ...
    for (const file of files) {
        try {
            const result = await processOCR(ai, file.fileData, file.mimeType, options, file.fileId, file.fileName);
            results.push({
                fileId: file.fileId,
                result
            });
