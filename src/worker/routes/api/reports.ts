...
async function calculateFinancialMetrics(
  ...
  const currentAssets = await getAccountBalancesByType(
    dbAdapter,
    entityId,
    "ASSET",
    "Current Asset",
  ...
  const totalAssets = await getAccountBalancesByType(
    dbAdapter,
    entityId,
    "ASSET",
    null,
  ...
  const currentLiabilities = await getAccountBalancesByType(
    dbAdapter,
    entityId,
    "LIABILITY",
    "Current Liability",
  ...
  const totalLiabilities = await getAccountBalancesByType(
    dbAdapter,
    entityId,
    "LIABILITY",
    null,
  ...
  const totalEquity = await getAccountBalancesByType(
    dbAdapter,
    entityId,
    "EQUITY",
    null,
  ...
  const cash = await getAccountBalancesByCategory(
    dbAdapter,
    entityId,
    "Cash",
    asOfDate
  );
  const inventory = await getAccountBalancesByCategory(
    dbAdapter,
    entityId,
    "Inventory",
    asOfDate
  );
  const receivables = await getAccountBalancesByCategory(
    dbAdapter,
    entityId,
    "Accounts Receivable",
    asOfDate
...
async function getAccountBalancesByType(
  _dbAdapter: DatabaseAdapter,
  _entityId: string,
  type: string,
  subtype: string | null,
  _asOfDate: Date
  ...
  try {
    ...
    console.error(
      `Error calculating balance for type ${type}, subtype ${subtype}:`,
      error
    );
    return 0;
  }
...
const setupReportsContext = createMiddleware<AppContext & ReportsContext>(
  async (c, next) => {
...
async function calculateCashRatio(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    const totalCash = await Promise.all(
      cashAccounts.map((account: any) =>
        Promise.resolve(account.currentBalance || 0)
      )
    ).then((balances: number[]) =>
      balances.reduce((sum: number, balance: number) => sum + balance, 0)
    );

    // Get current liabilities
    const liabilityAccounts = await dbAdapter.getAccountsByType("LIABILITY");
    const currentLiabilities = liabilityAccounts.reduce(
      (sum: number, account: any) => sum + (account.currentBalance || 0),
      0
    );
...
async function calculateTimesInterestEarned(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    console.error(
      "Error calculating times interest earned:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateReturnOnAssets(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    console.error(
      "Error calculating return on assets:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateReturnOnEquity(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    console.error(
      "Error calculating return on equity:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateGrossProfitMargin(
  ...
  try {
    ...
    console.error(
      "Error calculating gross profit margin:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateNetProfitMargin(
  ...
  try {
    ...
    console.error(
      "Error calculating net profit margin:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateAssetTurnover(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    console.error(
      "Error calculating asset turnover:",
      error instanceof Error ? error.message : String(error)
    );
    return 0;
  }
...
async function calculateInventoryTurnover(
  ...
  try {
    ...
    const averageInventory = await Promise.all(
      inventoryAccounts.map((account: any) =>
        Promise.resolve(account.currentBalance || 0)
      )
    ).then((balances: number[]) =>
      balances.reduce((sum: number, balance: number) => sum + balance, 0)
...
async function calculateReceivablesTurnover(
  dbAdapter: DatabaseAdapter,
  _entityId: string,
  _asOfDate: Date
): Promise<number> {
  try {
    ...
    const averageReceivables = await Promise.all(
      receivablesAccounts.map((account: any) =>
        Promise.resolve(account.currentBalance || 0)
      )
    ).then((balances: number[]) =>
      balances.reduce((sum: number, balance: number) => sum + balance, 0)
...
reportsRouter.get(
  "/trial-balance",
  async (c: Context<AppContext & ReportsContext>) => {
    try {
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      return c.json({
        ...
        data: {
          ...
          metrics: {
            ...
            workingCapital:
              balanceSheet.assets.current.reduce(
      ...
      console.error(
        "Error generating balance sheet:",
        error instanceof Error ? error.message : String(error)
      );
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      const incomeStatement = await _reportsEngine.generateIncomeStatement(
        startDate,
        endDate,
        entityId
      );
      ...
      const netIncomeMargin =
        grossRevenue > 0 ? (incomeStatement.netIncome / grossRevenue) * 100 : 0;
      ...
      return c.json({
        ...
        data: {
          ...
          metrics: {
            ...
            netIncomeMargin,
            expenseRatio:
              grossRevenue > 0 ? (totalExpenses / grossRevenue) * 100 : 0,
          },
          metadata: {
            generatedAt: new Date().toISOString(),
            generatedBy: c.get("user")?.id || "unknown",
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      // Generate cash flow statement
      const cashFlow = await _reportsEngine.generateCashFlowStatement(
        startDate,
        endDate,
        entityId
      );
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      const balanceSheet = await _reportsEngine.generateBalanceSheet(
        asOfDate,
        entityId
      );
      ...
      return c.json({
        ...
        data: {
          ...
          liquidity: {
            ...
            cashRatio: await calculateCashRatio(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
          ...
          leverage: {
            ...
            timesInterestEarned: await calculateTimesInterestEarned(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
          ...
          profitability: {
            returnOnAssets: await calculateReturnOnAssets(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
            returnOnEquity: await calculateReturnOnEquity(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
            grossProfitMargin: await calculateGrossProfitMargin(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
            netProfitMargin: await calculateNetProfitMargin(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
          ...
          activity: {
            assetTurnover: await calculateAssetTurnover(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
            ...
            receivablesTurnover: await calculateReceivablesTurnover(
              c.get("dbAdapter"),
              c.get("entityId"),
              asOfDate
            ),
      ...
      console.error(
        "Error generating financial metrics:",
        error instanceof Error ? error.message : String(error)
      );
      return c.json(
        {
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      const [balanceSheet, currentIncome, previousIncome, metrics] =
        await Promise.all([
          _reportsEngine.generateBalanceSheet(today, entityId),
          _reportsEngine.generateIncomeStatement(
            currentMonthStart,
            currentMonthEnd,
            entityId
          ),
      ...
      const revenueChange =
        currentIncome.revenue.total - previousIncome.revenue.total;
      const expenseChange =
        currentIncome.expenses.total - previousIncome.expenses.total;
        ...
        data: {
          ...
          trends: {
            revenueChange: {
              ...
              percentage:
                previousIncome.revenue.total > 0
            ...
            expenseChange: {
              ...
              percentage:
                previousIncome.expenses.total > 0
            ...
            netIncomeChange: {
              ...
              percentage:
                previousIncome.netIncome !== 0
                  ? (netIncomeChange / Math.abs(previousIncome.netIncome)) * 100
                  : 0,
            },
          },
          ratios: {
            ...
            debtToEquityRatio: metrics.debtToEquityRatio,
            netProfitMargin:
              currentIncome.revenue.total > 0
                ? (currentIncome.netIncome / currentIncome.revenue.total) * 100
                : 0,
          },
          metadata: {
            generatedAt: new Date().toISOString(),
      ...
      switch (format.toLowerCase()) {
        case "csv": {
          ...
          const csvLines = [
            ...
            ...balanceSheet.assets.current.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
            ...balanceSheet.assets.nonCurrent.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
            ...
            "LIABILITIES",
            "Account Code,Account Name,Balance",
            ...balanceSheet.liabilities.current.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
            ...balanceSheet.liabilities.nonCurrent.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
            ...
            "EQUITY",
            "Account Code,Account Name,Balance",
            ...balanceSheet.equity.accounts.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
          ...
          return new Response(csvContent, {
            headers: {
              ...
              "Content-Disposition": `attachment; filename="balance-sheet-${formatDate(
                asOfDate
              )}.csv"`,
            },
          });
...
<body>
    <div class="header">
        ...
        <p>Generated: ${formatDate(new Date())} by ${
    ...
    <div class="section">
        ...
        <table>
            ...
            ${balanceSheet.assets.nonCurrent
              ...
              .join("")}
            <tr class="total"><td colspan="2">TOTAL ASSETS</td><td class="amount">${formatCurrency(
    ...
    <div class="section">
        ...
        <table>
            ...
            ${balanceSheet.liabilities.nonCurrent
              ...
              .join("")}
            <tr class="total"><td colspan="2">TOTAL LIABILITIES</td><td class="amount">${formatCurrency(
    ...
    <div class="section">
        ...
        <table>
            ...
            ${balanceSheet.equity.accounts
              ...
              .join("")}
            <tr class="total"><td colspan="2">TOTAL EQUITY</td><td class="amount">${formatCurrency(
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr><td>Current Ratio</td><td class="amount">${metrics.currentRatio.toFixed(
              2
            )}</td></tr>
            <tr><td>Quick Ratio</td><td class="amount">${metrics.quickRatio.toFixed(
              2
            )}</td></tr>
            <tr><td>Debt-to-Equity Ratio</td><td class="amount">${metrics.debtToEquityRatio.toFixed(
              2
            )}</td></tr>
        </table>
    </div>
          ...
          return new Response(htmlContent, {
            headers: {
              ...
              "Content-Disposition": `inline; filename="balance-sheet-${formatDate(
                asOfDate
              )}.html"`,
            },
          });
      ...
      switch (format.toLowerCase()) {
        case "csv": {
          const csvLines = [
            ...
            ...incomeStatement.revenue.accounts.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
            ...
            "EXPENSES",
            "Account Code,Account Name,Amount",
            ...incomeStatement.expenses.accounts.map(
              (acc: any) =>
                `${acc.accountCode},${acc.accountName},${acc.currentBalance}`
            ),
          ...
          return new Response(csvContent, {
            headers: {
              "Content-Type": "text/csv",
              "Content-Disposition": `attachment; filename="income-statement-${formatDate(
                startDate
              )}-to-${formatDate(endDate)}.csv"`,
            },
          });
        }
        ...
        case "pdf":
          // Generate PDF format
          const pdfBuffer = generateIncomeStatementPDF(incomeStatement, {
            entityId,
            fromDate: startDate,
            toDate: endDate,
          });
          return new Response(pdfBuffer, {
            headers: {
              ...
              "Content-Disposition": `attachment; filename="income-statement-${
                startDate.toISOString().split("T")[0]
              }-to-${endDate.toISOString().split("T")[0]}.pdf"`,
            },
          });
          ...
          return c.json(
            {
              ...
              message:
                "Excel generation requires Node.js environment. Please use CSV or PDF format instead.",
              supportedFormats: ["csv", "json", "pdf"],
              alternativeFormats: {
                csv: `/api/reports/export/income-statement?format=csv&startDate=${formatDate(
                  startDate
                )}&endDate=${formatDate(endDate)}`,
                pdf: `/api/reports/export/income-statement?format=pdf&startDate=${formatDate(
                  startDate
                )}&endDate=${formatDate(endDate)}`,
              },
            },
...
reportsRouter.get(
  ...
  async (c: Context<AppContext & ReportsContext>) => {
    try {
      ...
      // Generate trial balance
      const trialBalance = await _reportsEngine.generateTrialBalance(
        asOfDate,
        entityId
      );
      ...
      switch (format.toLowerCase()) {
        case "csv": {
          const csvLines = [
            ...
            ...trialBalance.accounts.map((acc: any) => {
              const debit =
                ...
                (acc.accountType === "ASSET" || acc.accountType === "EXPENSE")
                  ? acc.currentBalance
                  : 0;
              const credit =
                acc.currentBalance > 0 &&
          ...
          return new Response(csvContent, {
            headers: {
              "Content-Type": "text/csv",
              "Content-Disposition": `attachment; filename="trial-balance-${formatDate(
                asOfDate
              )}.csv"`,
            },
          });
        ...
        case "pdf":
          // Generate PDF format
          const pdfBuffer = generateTrialBalancePDF(trialBalance, {
            entityId,
            asOfDate,
          });
          return new Response(pdfBuffer, {
            headers: {
              "Content-Type": "application/pdf",
              "Content-Disposition": `attachment; filename="trial-balance-${
          ...
          return c.json(
            {
              ...
              supportedFormats: ["csv", "json", "pdf"],
              alternativeFormats: {
                csv: `/api/reports/export/trial-balance?format=csv&asOfDate=${formatDate(
                  asOfDate
                )}`,
                pdf: `/api/reports/export/trial-balance?format=pdf&asOfDate=${formatDate(
                  asOfDate
                )}`,
              },
            },
