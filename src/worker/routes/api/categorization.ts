...
categorization.post("/suggest", async (c) => {
  try {
    ...
    const { description, amount, merchant, existingCategories, transactionId } =
      validatedData;
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Unknown error while finding matching account";
  ...
  try {
    ...
    if (approved && suggestion.transactionId && suggestion.suggestedAccountId) {
      try {
        ...
        console.warn(
          "Failed to update transaction with approved category:",
          error instanceof Error ? error.message : String(error)
        );
      }
    }
...
categorization.get("/history", async (c) => {
  try {
    ...
    console.error(
      "Failed to fetch categorization history:",
      error instanceof Error ? error.message : String(error)
    );
    return c.json(
      {
...
function getTopCategories(
  approvedSuggestions: CategorizationSuggestion[]
