...
export function DocumentSearch({ className = '' }) {
    ...
    return (<div className={`document-search ${className}`}>
      <Card className="p-6">
        <div className="space-y-4">
          ...
          {/* Search Stats */}
          {searchStats && (<div className="text-sm text-gray-600">
          ...
          {results.length > 0 && (<div className="space-y-3">
              ...
              {results.map((result) => (<Card key={result.id} className="p-4 hover:shadow-md transition-shadow">
                  <div className="space-y-2">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {result.mimeType && (<span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
                            {result.mimeType}
                          </span>)}
                      </div>
                      <div className="text-right">
                        ...
                        {result.uploadedAt && (<div className="text-xs text-gray-500">
                            {formatTimestamp(result.uploadedAt)}
                          </div>)}
                      </div>
                    </div>
                    
                    {result.matchedText && (<p className="text-gray-700 text-sm leading-relaxed">
                        {truncateText(result.matchedText)}
                      </p>)}
                    
                    {result.chunkInfo && (<div className="text-xs text-gray-500">
          ...
          {/* No Results */}
          {!loading && results.length === 0 && searchStats && (<div className="text-center py-8 text-gray-500">
