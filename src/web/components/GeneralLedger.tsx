...
// Extend or Pick from DomainAccount if the UI needs specific extra fields or a subset
// The review comment suggested: Pick<Account, ...> & { children?: ... }
// For now, let's try to use the DomainAccount directly or a type that matches the API response
// The local interface had:
//   id: number;
//   code: string;
//   name: string;
//   type: string;
...
//   path: string;
//   isActive: boolean;
//   balance?: number; // Domain has currentBalance
//   normalBalance: "debit" | "credit"; // Domain has NormalBalance ('DEBIT' | 'CREDIT')
//   createdAt: string; // Domain has Date
//   updatedAt: string; // Domain has Date

// Let's create a mapped type to handle the differences (like Date vs string from JSON) if necessary,
// or just use the domain type and cast/format where needed.
// Given strict TS, we might need a specific mapped type for the API response.

interface Account extends Omit<DomainAccount, 'createdAt' | 'updatedAt' | 'normalBalance' | 'type'> {
  type: string; // The local one used string, domain uses AccountType enum. keeping string for now to match select values or update select to use enum.
  // API likely returns ISO strings for dates
  createdAt: string;
  updatedAt: string;
  // Account for potential naming differences or extra fields in the specific API endpoint
  balance?: number;
...
export default function GeneralLedger() {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      const url = apiUrl(`/api/accounts?${params}`);
                ...
                {paginatedAccounts.length === 0 ? (
                  ...
                  paginatedAccounts.map((account) => (
                    <TableRow key={account.id}>
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${account.isActive
