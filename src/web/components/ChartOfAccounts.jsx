...
const API_BASE_URL = typeof import.meta.env !== 'undefined' && import.meta.env.PUBLIC_API_URL
...
export default function ChartOfAccounts() {
    ...
    const renderAccountRow = (account, level = 0) => {
        ...
        rows.push(<TableRow key={account.id} className={level > 0 ? 'bg-gray-50' : ''}>
        <TableCell style={{ paddingLeft: `${16 + indent}px` }}>
          <div className="flex items-center gap-2">
            {hasChildren && (<button onClick={() => toggleExpanded(account.id)} className="w-4 h-4 flex items-center justify-center text-gray-500 hover:text-gray-700">
                {isExpanded ? '−' : '+'}
              </button>)}
        ...
        <TableCell>
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${account.type === 'ASSET' ? 'bg-blue-100 text-blue-800' :
        ...
        <TableCell>
          <div className="flex gap-2">
            ...
            {!account.isSystem && (<Button variant="destructive" size="sm" onClick={() => handleDeleteRequest(account)}>
                Delete
              </Button>)}
          </div>
        </TableCell>
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {error && (<div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-md">
            ...
            <div className="w-48">
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  {ACCOUNT_TYPES.map(type => (<SelectItem key={type.value} value={type.value}>
                      {type.label}
                    </SelectItem>))}
                </SelectContent>
              </Select>
          ...
          <Table>
            ...
            <TableBody>
              {hierarchicalAccounts.length === 0 ? (<TableRow>
                  <TableCell colSpan={6} className="text-center text-gray-500 py-8">
                    No accounts found
                  </TableCell>
                </TableRow>) : (hierarchicalAccounts.map(account => renderAccountRow(account)))}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="code" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="e.g., 1000"/>
              </div>
              <div>
            ...
            <div>
              ...
              <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., Cash and Cash Equivalents"/>
            </div>
            ...
            .map(parent => (<SelectItem key={parent.id} value={parent.id.toString()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="subtype" value={formData.subtype} onChange={(e) => setFormData({ ...formData, subtype: e.target.value })} placeholder="Optional"/>
              </div>
              <div>
                <Label htmlFor="category">Category</Label>
                <Input id="category" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} placeholder="Optional"/>
              </div>
            </div>
            ...
            .map(account => (<SelectItem key={account.id} value={account.id}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-2">
                <input type="checkbox" id="allowTransactions" checked={formData.allowTransactions} onChange={(e) => setFormData({ ...formData, allowTransactions: e.target.checked })} className="rounded border-gray-300"/>
