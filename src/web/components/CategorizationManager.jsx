...
export default function CategorizationManager({ onSuggestionApproved, className = '' }) {
    ...
    if (suggestions.length === 0) {
        ...
        <div className="text-center py-8">
          ...
          <button onClick={loadSuggestions} className="mt-2 text-blue-600 hover:text-blue-800 text-sm">
            Refresh
          </button>
        </div>
      </div>);
    ...
    return (<div className={`p-4 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={loadSuggestions} disabled={loading} className="text-blue-600 hover:text-blue-800 text-sm disabled:opacity-50">
      ...
      <div className="space-y-4">
        {suggestions.map((suggestion) => {
            ...
            return (<div key={suggestion.id} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mb-3">
                    ...
                    {suggestion.reasoning && (<p className="text-xs text-gray-500 mt-1">
                ...
                <div className="flex gap-2 ml-4">
                  <button onClick={() => handleApprove(suggestion)} disabled={isProcessing} className="px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isProcessing ? '...' : '✓ Approve'}
                  </button>
                  
                  <button onClick={() => handleReject(suggestion)} disabled={isProcessing} className="px-3 py-1 bg-red-600 text-white text-sm rounded hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isProcessing ? '...' : '✗ Reject'}
                  </button>
                </div>
              </div>
