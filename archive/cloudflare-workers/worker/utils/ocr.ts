  ...
  try {
    if (!isOCRSupported(mimeType)) {
      ...
      return {
        valid: false,
        error,
        errorCode: OCR_ERROR_CODES.UNSUPPORTED_FORMAT,
      };
    }

    const maxSize =
      OCR_FILE_SIZE_LIMITS[mimeType as keyof typeof OCR_FILE_SIZE_LIMITS];
    ...
    if (fileSize < 100) {
      ...
      return {
        valid: false,
        error,
        errorCode: OCR_ERROR_CODES.VALIDATION_FAILED,
      };
    }
 ...
 */
function classifyOCRError(error: any): {
  code: string;
  message: string;
  retryable: boolean;
} {
...
function updateOCRMetrics(result: OCRResult, errorCode?: string) {
  ...
  if (result.success === true) {
    ...
    if (errorCode) {
      ocrMetrics.errorsByType[errorCode] =
        (ocrMetrics.errorsByType[errorCode] || 0) + 1;
    }
  }
  ...
  if (result.processingTime) {
    const totalTime =
      ocrMetrics.averageProcessingTime * (ocrMetrics.totalAttempts - 1) +
...
async function processFallbackOCR(
  ...
  try {
    ...
    const validation = validateOCRRequirements(
      mimeType,
      fileData.byteLength,
      undefined,
      logger
    ...
    if (!validation.valid) {
      return {
        success: false,
        error:
          validation.error || "File validation failed in fallback processing",
    ...
    const fallbackText =
      "OCR processing failed. Please manually extract text from this document or try uploading a clearer image.";
  ...
  try {
    ...
    if (aiResponse && typeof aiResponse === "object") {
      ...
      if (options.includeConfidence) {
        confidence =
          (aiResponse as any).confidence ||
          (aiResponse as any).result?.confidence ||
          0;
    ...
    if (!extractedText || extractedText.trim().length === 0) {
      ...
      return {
        success: false,
        error:
          "No text could be extracted from the document. The image may be too blurry, contain no text, or be in an unsupported format.",
        processingTime,
      };
    }
