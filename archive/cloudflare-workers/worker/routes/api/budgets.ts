...
function validateBudgetPeriodData(
  data: CreateBudgetPeriodRequest
): string | null {
  if (
    !data.name ||
    ...
    const result =
      conditions.length > 0
        ? await baseQuery
            ...
            .orderBy(
              desc(budgetPeriods.fiscalYear),
              desc(budgetPeriods.startDate)
            )
...
budgetsRouter.post("/periods", async (c) => {
  try {
    ...
    if (overlapping.length > 0) {
      return c.json(
        { success: false, error: "Period overlaps with existing period" },
        400
      );
    }
...
budgetsRouter.delete("/periods/:id", async (c) => {
  try {
    ...
    const associatedBudgets = await db
      .select()
      .from(budgets)
      ...
      const budgetsWithAllocations = await Promise.all(
        result.map(async (item) => {
          const allocations = await db
            ...
            .leftJoin(
              categories,
              eq(budgetAllocations.categoryId, categories.id)
            )
...
budgetsRouter.put("/:id", async (c) => {
  try {
    ...
    if (existingBudget.status === "locked") {
      return c.json(
        { success: false, error: "Cannot modify locked budget" },
        400
      );
    }
    ...
    const hasSignificantChanges =
      (body.totalAmount !== undefined &&
        body.totalAmount !== existingBudget.plannedAmount) ||
    ...
    if (hasSignificantChanges) {
      ...
      const latestRevision = await db
        .select({ revisionNumber: budgetRevisions.revisionNumber })
        .from(budgetRevisions)
        .where(eq(budgetRevisions.budgetId, id))
      ...
      const nextRevisionNumber =
        latestRevision.length > 0 ? latestRevision[0].revisionNumber + 1 : 1;
    ...
    if (body.totalAmount !== undefined)
      updateData.plannedAmount = body.totalAmount;
...
budgetsRouter.delete("/:id", async (c) => {
  try {
    ...
    // Delete revisions
    await db.delete(budgetRevisions).where(eq(budgetRevisions.budgetId, id));
  ...
  try {
    ...
    if (!body.categoryId || !body.allocatedAmount || body.allocatedAmount < 0) {
      return c.json(
        {
          success: false,
          error: "Valid category ID and allocated amount are required",
        },
        400
      );
    }
    ...
    if (existing.length > 0) {
      return c.json(
        {
          success: false,
          error: "Allocation already exists for this category",
        },
        400
      );
    }
    ...
    const allocationSummary =
      allocationConditions.length > 0
        ? await allocationQuery
            ...
            .groupBy(budgetAllocations.categoryId)
