...
function validatePasswordDetailed(password: string): {
  isValid: boolean;
  errors: string[];
  score: number;
} {
...
authRouter.post("/register", async (c) => {
  ...
  try {
    ...
    // Registration error occurred
    return c.json(
      { success: false, error: "Registration failed", message: errorMessage },
      500
    );
  }
});
...
authRouter.post("/login", async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: false, error: "Login failed", message: errorMessage },
      500
    );
  }
});
...
authRouter.post("/logout", authMiddleware, async (c) => {
  try {
    ...
    return c.json(
      { success: false, error: "Logout failed", message: errorMessage },
      500
    );
  }
});
...
authRouter.put("/profile", authMiddleware, async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: false, error: "Profile update failed", message: errorMessage },
      500
    );
  }
});
...
authRouter.put("/password", authMiddleware, async (c) => {
  ...
  try {
    ...
    await db
      .update(users)
      .set({ passwordHash: hashedNewPassword.combined, updatedAt: new Date() })
      .where(eq(users.id, user.id));
...
authRouter.get("/validate", authMiddleware, async (c) => {
  ...
  if (!user) {
    return c.json(
      { success: false, error: "Unauthorized", message: "Invalid session" },
      401
    );
  }
...
authRouter.post(
  ...
  async (c) => {
    try {
      ...
      } = await import("../../../lib/auth/magicLink");
      const { MagicLinkPurpose } = await import("../../../lib/auth/types");

      // Create magic link manager and rate limiter
      const magicLinkManager = createMagicLinkManager(
        c.env.FINANCE_MANAGER_CACHE,
        {
          defaultTTLMinutes: 15,
        }
      );

      const rateLimiter = createMagicLinkRateLimiter(
        c.env.FINANCE_MANAGER_CACHE,
        {
        ...
        return c.json(
          {
            ...
            message: `Too many requests. Try again in ${Math.ceil(
              (rateLimitResult.resetTime.getTime() - Date.now()) / 1000 / 60
            )} minutes.`,
      ...
      // Generate magic link
      const { token } = await magicLinkManager.generateMagicLink(
        email,
...
authRouter.post(
  ...
  async (c) => {
    try {
      ...
      // Import magic link utilities
      const { createMagicLinkManager } = await import(
        "../../../lib/auth/magicLink"
      );
      const { MagicLinkPurpose } = await import("../../../lib/auth/types");
      ...
      switch (linkPurpose) {
        ...
        default:
          return c.json(
            { error: "Invalid purpose", message: "Unknown magic link purpose" },
            400
          );
      }
    } catch (error) {
