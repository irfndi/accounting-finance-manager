...
notificationsRouter.post(
  "/send-bulk",
  authMiddleware,
  zValidator("json", sendBulkEmailSchema),
  async (c) => {
    try {
      const emailService = createEmailService(c.env);
...
notificationsRouter.post(
  "/send-template",
  authMiddleware,
  zValidator("json", sendTemplateEmailSchema),
  async (c) => {
    ...
    try {
      ...
      const emailSubject =
        subject ||
        extractSubjectFromTemplate(renderedContent) ||
  ...
  return {
    ...
    htmlBody: `
      ...
      <table style="border-collapse: collapse; width: 100%;">
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Account:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">${
          account.name
        }</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Type:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">${
          transaction.type
        }</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Amount:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">$${
          transaction.amount
        }</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Date:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">${
          transaction.date
        }</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Description:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">${
...
function generateBudgetWarningEmail(data: any) {
  ...
  return {
    ...
    htmlBody: `
      ...
      <p>Your budget "${
        budget.name
      }" has reached ${percentage}% of its limit.</p>
      <table style="border-collapse: collapse; width: 100%;">
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Budget:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">${
          budget.name
        }</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Limit:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">$${
          budget.amount
        }</td></tr>
        ...
        <tr><td style="border: 1px solid #ddd; padding: 8px;"><strong>Remaining:</strong></td><td style="border: 1px solid #ddd; padding: 8px;">$${
      ...
      <p style="color: ${
        percentage >= 90 ? "red" : "orange"
      };">Please review your spending to stay within budget.</p>
    `,
    textBody: `
Budget Warning
...
${activities
  .map(
    (activity: any) =>
      `- ${activity.date}: ${activity.description} - $${activity.amount}`
  )
  .join("\n")}
...
function initializeEmailTemplates(): void {
  ...
  if (EmailTemplateManager.getAvailableTemplates().length === 0) {
    ...
    EmailTemplateManager.registerTemplate(
      "transaction-alert",
      getTransactionAlertTemplate()
    );
    EmailTemplateManager.registerTemplate(
      "budget-alert",
    ...
    EmailTemplateManager.registerTemplate(
      "magic-link-login",
      getMagicLinkLoginTemplate()
    );
    EmailTemplateManager.registerTemplate(
      "magic-link-register",
      getMagicLinkRegisterTemplate()
    );
    EmailTemplateManager.registerTemplate(
      "magic-link-verify-email",
      getMagicLinkVerifyEmailTemplate()
    );
    EmailTemplateManager.registerTemplate(
      "magic-link-reset-password",
      getMagicLinkResetPasswordTemplate()
    );
