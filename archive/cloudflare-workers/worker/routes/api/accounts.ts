...
function validateNormalBalance(
...
async function createAccountingServices(
...
accounts.get("/", async (c) => {
  try {
    ...
    const { accountRegistry } = await createAccountingServices(
      c.env.FINANCE_MANAGER_DB,
      entityId
    );
    ...
    if (type) {
      allAccounts = await accountRegistry.getAccountsByTypeFromDatabase(
    ...
    if (active !== undefined) {
      ...
      allAccounts = allAccounts.filter(
        (account: CoreAccount) => account.isActive === isActive
      );
    }
    ...
    if (parent) {
      ...
      if (!Number.isNaN(parentId)) {
        allAccounts = allAccounts.filter(
          (account: CoreAccount) => account.parentId === parentId
        );
      }
    }
    ...
    const enhancedAccounts = allAccounts.map((account: CoreAccount) => ({
      ...
      formattedBalance: account.currentBalance
        ? formatCurrency(account.currentBalance, "USD")
        : null,
      accountingInfo: {
        canHaveChildren: ["ASSET", "LIABILITY", "EQUITY"].includes(
          account.type
        ),
...
accounts.get("/:id", async (c) => {
  try {
    ...
    const { dbAdapter, accountRegistry } = await createAccountingServices(
      c.env.FINANCE_MANAGER_DB,
      entityId
    );
    ...
    const childAccounts = accountRegistry
      .getAllAccounts()
      .filter((acc: CoreAccount) => acc.parentId === accountId);
    ...
    const enhancedAccount = {
      ...
      children: childAccounts.map((child: CoreAccount) => ({
        ...
        formattedBalance: child.currentBalance
          ? formatCurrency(child.currentBalance, "USD")
          : null,
      })),
      accountingInfo: {
        canHaveChildren: ["ASSET", "LIABILITY", "EQUITY"].includes(
...
accounts.post("/", async (c) => {
  try {
    const body: CreateAccountRequest & { entityId?: string } =
      await c.req.json();
    ...
    const normalBalanceError = validateNormalBalance(
      body.normalBalance,
      body.type
    );
    if (normalBalanceError) {
      return c.json(
        { error: normalBalanceError, code: "VALIDATION_ERROR" },
        400
      );
    }
    ...
    const duplicateAccount = existingAccounts.find(
      (acc: CoreAccount) => acc.code === body.code
    );
    if (duplicateAccount) {
      return c.json(
        {
    ...
    const accountData: Omit<CoreAccount, "id" | "createdAt" | "updatedAt"> = {
      ...
      normalBalance:
        (body.normalBalance as NormalBalance) ||
        getNormalBalance(body.type as AccountType),
      currentBalance: 0,
    ...
    return c.json(
      {
        account: {
          ...
          accountingInfo: {
            ...
            isBalanceSheet: ["ASSET", "LIABILITY", "EQUITY"].includes(
              newAccount.type
            ),
...
accounts.put("/:id", async (c) => {
  try {
    ...
    const { dbAdapter } = await createAccountingServices(
      c.env.FINANCE_MANAGER_DB
    );
...
accounts.delete("/:id", async (c) => {
  try {
    ...
    const transactionCount = hasTransactions
      ? Number(hasTransactions.count)
      : 0;
