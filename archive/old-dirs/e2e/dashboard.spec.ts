...
test.describe("Dashboard", () => {
  ...
  test("should display dashboard title and description", async ({
    ...
    await expect(
      page.locator('[data-testid="dashboard-description"]')
    ).toContainText("Monitor your financial performance");
  });
  ...
  test("should display financial summary cards", async ({
    ...
    await expect(
      page.locator('[data-testid="total-assets-card"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="total-liabilities-card"]')
    ).toBeVisible();
  ...
  test("should display navigation modules", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    await expect(
      page.locator('[data-testid="general-ledger-module"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="ai-categorization-module"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="financial-reports-module"]')
    ).toBeVisible();
  });

  test("should navigate to different modules", async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test("should display AI insights section", async ({
    page,
  }: {
    page: Page;
  }) => {
    // Wait for AI Insights module to be visible
    await page.waitForSelector('[data-testid="ai-insights-module"]', {
      timeout: 15000,
    });
    ...
    await page.waitForSelector('[data-testid="ai-insights-content"]', {
      timeout: 30000,
    });

    // Check that the AI insights title is visible
    await expect(
      page.locator('[data-testid="ai-insights-title"]')
    ).toContainText("AI Financial Insights");

    // Check that insights are displayed or loading state is shown
    const insightsVisible = await page
      .locator('[data-testid="ai-insights-content"]')
      .isVisible();
  ...
  test("should display quick actions", async ({ page }: { page: Page }) => {
    ...
    await expect(
      page.locator('[data-testid="add-transaction-button"]')
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.locator('button:has-text("Budget Review")')
    ).toBeVisible();
  });
  ...
  test("should handle period selector", async ({ page }: { page: Page }) => {
    ...
    if (await periodSelector.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="period-option-month"]')
      ).toBeAttached();
      await expect(
        page.locator('[data-testid="period-option-quarter"]')
      ).toBeAttached();
      await expect(
        page.locator('[data-testid="period-option-year"]')
      ).toBeAttached();
  ...
  test("should display export report button", async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test("should display AI insights toggle", async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test("should display error handling", async ({ page }: { page: Page }) => {
    ...
    await page.waitForSelector('[data-testid="ai-error-message-panel"]', {
      timeout: 10000,
    });
    // Check for error message in the AI insights panel
    await expect(
      page.locator('[data-testid="ai-error-message-panel"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="ai-error-message-panel"]')
    ).toContainText("Failed to load AI insights. Please try again later.");
  });
});
