...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    ...
    const { accountRegistry } = await createAccountingServices(
      runtime.env.FINANCE_MANAGER_DB
    );

    const account = await accountRegistry.getAccountById(
    ...
    return new Response(JSON.stringify({ account: enhancedAccount }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    const existingAccount = await accountRegistry.getAccountById(
      Number.parseInt(accountId, 10)
    );
    if (!existingAccount) {
      return new Response(JSON.stringify({ error: "Account not found" }), {
    ...
    const updatedAccount = await accountRegistry.updateAccount(
      existingAccount.id,
      updateData
    );
      ...
      JSON.stringify({
        account: {
          ...
          accountingInfo: {
            ...
            isIncomeStatement: ["REVENUE", "EXPENSE"].includes(
              updatedAccount.type
            ),
          },
        },
...
export const DELETE: APIRoute = async ({ params, locals }) => {
  try {
    ...
    const hasChildren = allAccounts.some(
      (account) => account.parentId === existingAccount.id
    );
    if (hasChildren) {
      return new Response(
