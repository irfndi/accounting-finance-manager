...
export async function generateIncomeStatementPDF(
  incomeStatement: any,
  options: { entityId: string; fromDate: Date; toDate: Date }
): Promise<ArrayBuffer> {
  try {
    const doc = new jsPDF();
    ...
    doc.text(
      `Period: ${options.fromDate.toISOString().split("T")[0]} to ${
    ...
    doc.text(
      `${(incomeStatement.totalRevenue || 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })}`,
      150,
    ...
    doc.text(
      `${(incomeStatement.totalExpenses || 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })}`,
      150,
    ...
    doc.text(
      `${(incomeStatement.netIncome || 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })}`,
      150,
...
export async function generateTrialBalancePDF(
  trialBalance: any,
  options: { entityId: string; asOfDate: Date }
): Promise<ArrayBuffer> {
  try {
    const doc = new jsPDF();
    ...
    doc.text(
      `${(trialBalance.totals?.totalDebits || 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })}`,
      120,
    ...
    doc.text(
      `${(trialBalance.totals?.totalCredits || 0).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      })}`,
      160,
...
export async function generateTrialBalanceExcelOriginal(
  trialBalance: any,
  options: { entityId: string; asOfDate: Date }
): Promise<ArrayBuffer> {
  try {
    ...
    workbook.SetCellValue(
      sheetName,
      "A3",
      `As of: ${options.asOfDate.toISOString().split("T")[0]}`
    );
    ...
    workbook.SetCellValue(
      sheetName,
      `B${currentRow}`,
      trialBalance.totals?.totalDebits || 0
    );
    workbook.SetCellValue(
      sheetName,
      `C${currentRow}`,
      trialBalance.totals?.totalCredits || 0
    );
