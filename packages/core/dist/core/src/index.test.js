...
import { FINANCIAL_CONSTANTS, formatCurrency, roundToDecimalPlaces, getNormalBalance, TransactionValidator, TransactionBuilder, BalanceCalculator, AccountingEngine, AccountRegistry, DoubleEntryError, JournalEntryManager, BalanceSheetError, AccountRegistryError, CurrencyConversionError, PeriodClosureError, FiscalYearError, ComplianceError, ErrorAggregator, AccountingErrorFactory, ErrorRecoveryManager,
// TODO: Re-enable when database adapters are fixed
// DatabaseAdapter,
...
describe('TransactionValidator', () => {
    ...
    describe('validateTransactionData', () => {
        ...
        it('should reject transaction without date', () => {
            ...
            const errors = TransactionValidator.validateTransactionData(invalidData);
...
describe('BalanceCalculator', () => {
    describe('calculateAccountBalance', () => {
        it('should calculate debit account balance correctly', () => {
            const balance = BalanceCalculator.calculateAccountBalance('ASSET', 'DEBIT', 1000, 300);
            expect(balance).toBe(700);
        });
        it('should calculate credit account balance correctly', () => {
            const balance = BalanceCalculator.calculateAccountBalance('LIABILITY', 'CREDIT', 200, 1000);
            expect(balance).toBe(800);
        });
        it('should handle zero balances', () => {
            const balance = BalanceCalculator.calculateAccountBalance('ASSET', 'DEBIT', 500, 500);
            expect(balance).toBe(0);
        });
        it('should handle negative balances for debit accounts', () => {
            const balance = BalanceCalculator.calculateAccountBalance('ASSET', 'DEBIT', 300, 1000);
            expect(balance).toBe(-700);
        });
        it('should handle negative balances for credit accounts', () => {
            const balance = BalanceCalculator.calculateAccountBalance('LIABILITY', 'CREDIT', 1000, 300);
            expect(balance).toBe(-700);
        });
    });
});
...
describe('Enhanced Error Handling System', () => {
    ...
    describe('ErrorAggregator', () => {
        ...
        it('should add and categorize errors by severity', () => {
            const warning = AccountingErrorFactory.createValidationError('test', 'Warning message', 'WARNING_CODE', 'WARNING');
            const error = AccountingErrorFactory.createValidationError('test', 'Error message', 'ERROR_CODE', 'ERROR');
            const critical = AccountingErrorFactory.createValidationError('test', 'Critical message', 'CRITICAL_CODE', 'CRITICAL');
        ...
        it('should filter errors by category', () => {
            const validationError = AccountingErrorFactory.createValidationError('test', 'Validation error', 'VALIDATION_ERROR', 'ERROR', 'VALIDATION');
            const businessError = AccountingErrorFactory.createBusinessRuleError('test', 'Business rule error', 'BUSINESS_ERROR');
        ...
        it('should clear all errors', () => {
            aggregator.addError(AccountingErrorFactory.createValidationError('test', 'Error', 'ERROR_CODE'));
            expect(aggregator.hasErrors()).toBe(true);
            aggregator.clear();
    ...
    describe('AccountingErrorFactory', () => {
        it('should create validation error with default values', () => {
            const error = AccountingErrorFactory.createValidationError('testField', 'Test message', 'TEST_CODE');
        ...
        it('should create validation error with custom values', () => {
            ...
            const error = AccountingErrorFactory.createValidationError('testField', 'Test message', 'TEST_CODE', 'CRITICAL', 'SYSTEM', suggestions, context);
        ...
        it('should create business rule error', () => {
            ...
            const error = AccountingErrorFactory.createBusinessRuleError('businessField', 'Business rule violated', 'BUSINESS_VIOLATION', suggestions);
            expect(error.category).toBe('BUSINESS_RULE');
            expect(error.severity).toBe('ERROR');
            expect(error.suggestions).toEqual(suggestions);
        });
        it('should create compliance error', () => {
            const error = AccountingErrorFactory.createComplianceError('complianceField', 'Compliance violated', 'COMPLIANCE_VIOLATION');
            expect(error.category).toBe('COMPLIANCE');
            expect(error.severity).toBe('CRITICAL');
        });
        it('should create system error', () => {
            const context = { systemId: 'SYS001' };
            const error = AccountingErrorFactory.createSystemError('systemField', 'System error', 'SYSTEM_ERROR', context);
            expect(error.category).toBe('SYSTEM');
            expect(error.severity).toBe('CRITICAL');
            expect(error.context).toEqual(context);
        });
    ...
    it('should generate report with detailed error information', () => {
        const aggregator = new ErrorAggregator();
        const error1 = AccountingErrorFactory.createValidationError('amount', 'Invalid amount', 'INVALID_AMOUNT', 'ERROR', 'VALIDATION', ['Check amount format']);
        const error2 = AccountingErrorFactory.createBusinessRuleError('account', 'Account not found', 'ACCOUNT_NOT_FOUND', ['Verify account exists']);
...
describe('D1 Database Integration', () => {
    ...
    const createMockD1Database = () => {
        ...
        return {
            ...
            setMockResults: (query, results) => {
                mockResults.set(query, results);
            }
        };
    };
    // TODO: Re-enable when database adapters are fixed
    describe.skip('DatabaseAdapter', () => {
        let dbAdapter;
        let mockDb;
        beforeEach(() => {
        ...
        it('should create account in database', async () => {
            ...
            // Set up mock to return the account data for any INSERT query
        ...
        it('should create transaction in database', async () => {
            ...
            // Set up mock to return transaction data for any query
            mockDb.prepare = vi.fn().mockReturnValue({
                bind: vi.fn().mockReturnValue({
                    all: vi.fn().mockResolvedValue({ results: [] }),
                    first: vi.fn().mockResolvedValue(mockTransaction),
                    run: vi.fn().mockResolvedValue({ success: true, meta: { changes: 1, last_row_id: 1 } })
                })
            });
            const transactionData = {
        ...
        it('should create journal entries in database', async () => {
            ...
            // Set up mock to return journal entry data for any query
            mockDb.prepare = vi.fn().mockReturnValue({
                bind: vi.fn().mockReturnValue({
                    all: vi.fn().mockResolvedValue({ results: [mockEntry] }),
                    first: vi.fn().mockResolvedValue(mockEntry),
                    run: vi.fn().mockResolvedValue({ success: true, meta: { changes: 1, last_row_id: 1 } })
                })
            });
    ...
    describe.skip('DatabaseAccountRegistry', () => {
        let registry;
        let mockDbAdapter;
        beforeEach(() => {
            mockDbAdapter = {
    ...
    describe.skip('DatabaseJournalEntryManager', () => {
        let manager;
        let mockDbAdapter;
        ...
        it('should create and persist transaction with journal entries', async () => {
            ...
            const transactionData = {
                ...
                currency: 'IDR',
                entries: [
                    { accountId: 1000, debitAmount: 1000, creditAmount: 0 },
                    { accountId: 2000, debitAmount: 0, creditAmount: 1000 }
                ],
                createdBy: 'test-user'
            };
