...
import { FinancialAIService, AIService, createProvider } from '@finance-manager/ai';
import { DatabaseAdapter } from '@finance-manager/core';
import { z } from 'zod';
// Validation schemas
...
categorization.post('/suggest', async (c) => {
    try {
        ...
        const aiResult = await financialAI.categorizeExpense(description, amount, merchant, existingCategories);
        ...
        try {
            ...
            const matchingAccount = accounts.find((account) => account.category?.toLowerCase().includes(lowerCaseCategory) ||
        ...
        await c.env.FINANCE_MANAGER_CACHE.put(kvKey, JSON.stringify(suggestion), { expirationTtl: 86400 * 7 } // 7 days expiration
        );
...
categorization.post('/approve', async (c) => {
    try {
        ...
        // Update in KV
        await c.env.FINANCE_MANAGER_CACHE.put(kvKey, JSON.stringify(suggestion), { expirationTtl: 86400 * 30 } // Keep approved/rejected for 30 days for analytics
        );
...
function getTopCategories(approvedSuggestions) {
