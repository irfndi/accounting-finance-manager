...
import { DatabaseAdapter, FinancialReportsEngine, formatCurrency, FINANCIAL_CONSTANTS, generateIncomeStatementPDF, generateIncomeStatementExcel, generateTrialBalancePDF, generateTrialBalanceExcel, } from '@finance-manager/core';
...
async function calculateCashRatio(dbAdapter, entityId, asOfDate) {
    try {
        // Get cash and cash equivalent accounts
        const cashAccounts = await dbAdapter.getAccountsByType('ASSET');
        const totalCash = await Promise.all(cashAccounts.map((account) => Promise.resolve(account.currentBalance || 0))).then((balances) => balances.reduce((sum, balance) => sum + balance, 0));
        // Get current liabilities
        const liabilityAccounts = await dbAdapter.getAccountsByType('LIABILITY');
        const currentLiabilities = liabilityAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
...
async function calculateTimesInterestEarned(dbAdapter, entityId, asOfDate) {
    try {
        ...
        const totalRevenue = revenueAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
        const totalExpenses = expenseAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
...
async function calculateReturnOnAssets(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
        ...
        const totalExpenses = expenseAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
        const netIncome = totalRevenue - totalExpenses;
        const totalAssets = assetAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
...
async function calculateReturnOnEquity(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
        ...
        const totalExpenses = expenseAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
        const netIncome = totalRevenue - totalExpenses;
        const totalEquity = equityAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
...
async function calculateGrossProfitMargin(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
        // Calculate revenue and cost of goods sold from account balances
        const revenueAccounts = await dbAdapter.getAccountsByType('REVENUE');
        const expenseAccounts = await dbAdapter.getAccountsByType('EXPENSE');
        const revenue = revenueAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0);
        // Estimate COGS as a portion of total expenses (would need specific account filtering in production)
        const costOfGoodsSold = expenseAccounts.reduce((sum, account) => sum + (account.currentBalance || 0), 0) * 0.6;
...
async function calculateNetProfitMargin(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
...
async function calculateAssetTurnover(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
...
async function calculateInventoryTurnover(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
        ...
        if (inventoryAccounts.length === 0)
            return 0;
        const averageInventory = await Promise.all(inventoryAccounts.map((account) => Promise.resolve(account.currentBalance || 0))).then((balances) => balances.reduce((sum, balance) => sum + balance, 0));
...
async function calculateReceivablesTurnover(dbAdapter, entityId, asOfDate) {
    try {
        const startOfYear = new Date(asOfDate.getFullYear(), 0, 1);
        ...
        if (receivablesAccounts.length === 0)
            return 0;
        const averageReceivables = await Promise.all(receivablesAccounts.map((account) => Promise.resolve(account.currentBalance || 0))).then((balances) => balances.reduce((sum, balance) => sum + balance, 0));
        ...
        switch (format.toLowerCase()) {
            case 'csv': {
                ...
                const csvLines = [
                    ...
                    ...balanceSheet.assets.nonCurrent.map((acc) => `${acc.accountCode},${acc.accountName},${acc.currentBalance}`),
                    ...
                    ...balanceSheet.liabilities.nonCurrent.map((acc) => `${acc.accountCode},${acc.accountName},${acc.currentBalance}`),
...
<body>
    ...
    <div class="section">
        ...
        <table>
            ...
            ${balanceSheet.assets.nonCurrent.map((acc) => `<tr><td>${acc.accountCode}</td><td>${acc.accountName}</td><td class="amount">${formatCurrency(acc.currentBalance)}</td></tr>`).join('')}
    ...
    <div class="section">
        ...
        <table>
            ...
            ${balanceSheet.liabilities.nonCurrent.map((acc) => `<tr><td>${acc.accountCode}</td><td>${acc.accountName}</td><td class="amount">${formatCurrency(acc.currentBalance)}</td></tr>`).join('')}
