...
import { budgetPeriods, budgets, budgetRevisions, budgetAllocations, categories, createDatabase } from '@finance-manager/db';
...
const budgetStatuses = ['draft', 'active', 'locked', 'archived'];
const approvalStatuses = ['pending', 'approved', 'rejected'];
// Validation functions
function validateBudgetPeriodData(data) {
...
budgetsRouter.post('/periods', async (c) => {
    try {
        ...
        const overlapping = await db.select()
            .from(budgetPeriods)
            .where(and(eq(budgetPeriods.periodType, body.type), gte(budgetPeriods.endDate, body.startDate), lte(budgetPeriods.startDate, body.endDate)));
...
budgetsRouter.get('/', async (c) => {
    try {
        ...
        if (includeAllocations === 'true') {
            const budgetsWithAllocations = await Promise.all(result.map(async (item) => {
...
budgetsRouter.put('/:id', async (c) => {
    try {
        ...
        const hasSignificantChanges = ((body.totalAmount !== undefined && body.totalAmount !== existingBudget.plannedAmount) ||
...
budgetsRouter.post('/:id/allocations', async (c) => {
    try {
        ...
        const existing = await db.select()
            .from(budgetAllocations)
            .where(and(eq(budgetAllocations.budgetId, budgetId), eq(budgetAllocations.categoryId, body.categoryId)));
        if (existing.length > 0) {
