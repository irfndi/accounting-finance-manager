...
import { rawDocs, OCRStatus, DocumentType } from "@finance-manager/db";
...
export async function createRawDoc(db, data) {
    ...
    try {
        const result = await withOCRErrorBoundary('Create Raw Document', async () => {
            const now = new Date();
 ...
 */
export async function updateRawDocOCR(db, fileId, ocrData, updatedBy) {
    const logger = createOCRLogger({
        fileId,
        userId: updatedBy
    });
    try {
        const result = await withOCRErrorBoundary('Update Raw Document OCR', async () => {
            const now = new Date();
        ...
        }, logger, { operation: 'UPDATE_OCR_DATA' });
        if (!result.success) {
 ...
 */
export async function updateRawDocLLM(db, fileId, llmData, updatedBy) {
    const logger = createOCRLogger({
        fileId,
        userId: updatedBy
    });
    try {
        const result = await withOCRErrorBoundary('Update Raw Document LLM', async () => {
            const now = new Date();
            const updateData = {
            ...
            return updatedDocs[0];
        }, logger, { operation: 'UPDATE_LLM_DATA' });
        if (!result.success) {
...
export async function getRawDocByFileId(db, fileId) {
    ...
    try {
        const result = await withOCRErrorBoundary('Get Raw Document by File ID', async () => {
            ...
            return docs[0];
        }, logger, { operation: 'GET_BY_FILE_ID' });
        if (!result.success) {
 ...
 */
export async function searchRawDocs(db, filters = {}, limit = 50, offset = 0) {
    const logger = createOCRLogger({
        userId: filters.userId,
        operation: 'SEARCH_DOCUMENTS'
    });
    try {
        const result = await withOCRErrorBoundary('Search Raw Documents', async () => {
            ...
            if (filters.tags && filters.tags.length > 0) {
                const tagConditions = filters.tags.map(tag => like(rawDocs.tags, `%${tag}%`));
                conditions.push(or(...tagConditions));
            }
            if (filters.dateFrom) {
                conditions.push(eq(rawDocs.createdAt, Math.floor(filters.dateFrom.getTime() / 1000)));
            }
            if (filters.dateTo) {
                conditions.push(eq(rawDocs.createdAt, Math.floor(filters.dateTo.getTime() / 1000)));
            }
 ...
 */
export async function deleteRawDoc(db, fileId, deletedBy) {
    const logger = createOCRLogger({
        fileId,
        userId: deletedBy
    });
    try {
        const result = await withOCRErrorBoundary('Delete Raw Document', async () => {
        ...
        }, logger, { operation: 'DELETE_DOCUMENT' });
        if (!result.success) {
 ...
 */
export function generateSearchableText(extractedText) {
 ...
 */
export async function getOCRStats(db, userId) {
    const logger = createOCRLogger({
        userId,
        operation: 'GET_OCR_STATS'
    });
    try {
        const result = await withOCRErrorBoundary('Get OCR Statistics', async () => {
            let query = db.select().from(rawDocs);
            if (userId) {
            ...
            if (completedDocs.length > 0) {
                const totalProcessingTime = completedDocs.reduce((sum, doc) => sum + (doc.ocrProcessingTime || 0), 0);
                stats.avgProcessingTime = totalProcessingTime / completedDocs.length;
                const totalConfidence = completedDocs.reduce((sum, doc) => sum + (doc.ocrConfidence || 0), 0);
                stats.avgConfidence = totalConfidence / completedDocs.length;
                stats.totalTextLength = completedDocs.reduce((sum, doc) => sum + (doc.textLength || 0), 0);
            }
            return stats;
        }, logger, { operation: 'GET_STATS' });
        if (!result.success) {
