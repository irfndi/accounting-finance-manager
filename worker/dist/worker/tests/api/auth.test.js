...
const createAuthenticatedRequest = (method, url, token, options = {}) => {
    return new Request(url, {
        method,
        headers: {
            'Content-Type': 'application/json',
...
const createTestJWTToken = (payload) => {
...
describe('Authentication API', () => {
    ...
    describe('POST /api/auth/register', () => {
        it('should register a new user successfully', async () => {
            ...
            const request = createTestRequest('POST', 'https://example.com/api/auth/register', { body: JSON.stringify(userData) });
    ...
    describe('POST /api/auth/login', () => {
        it('should login user with valid credentials', async () => {
            ...
            const request = createTestRequest('POST', 'https://example.com/api/auth/login', { body: JSON.stringify(loginData) });
            const response = await app.fetch(request, env);
    ...
    describe('GET /api/auth/profile', () => {
        it('should return user profile with valid token', async () => {
            ...
            const request = createAuthenticatedRequest('GET', 'https://example.com/api/auth/profile', token);
            const response = await app.fetch(request, env);
            expect(response.status).toBe(200);
            const result = await parseJsonResponse(response);
            expect(result.user).toBeDefined();
        ...
        it('should reject request without token', async () => {
            const request = createTestRequest('GET', 'https://example.com/api/auth/profile');
            const response = await app.fetch(request, env);
            expect(response.status).toBe(401);
            const result = await parseJsonResponse(response);
        ...
        it('should reject request with invalid token', async () => {
            const request = createAuthenticatedRequest('GET', 'https://example.com/api/auth/profile', 'invalid.token.here');
            const response = await app.fetch(request, env);
            expect(response.status).toBe(401);
            const result = await parseJsonResponse(response);
    ...
    describe('PUT /api/auth/profile', () => {
        it('should update user profile successfully', async () => {
            ...
            const request = createAuthenticatedRequest('PUT', 'https://example.com/api/auth/profile', token, { body: JSON.stringify(updateData) });
            const response = await app.fetch(request, env);
            expect(response.status).toBe(200);
            const result = await parseJsonResponse(response);
    ...
    describe('POST /api/auth/logout', () => {
        it('should logout user successfully', async () => {
            ...
            const request = createAuthenticatedRequest('POST', 'https://example.com/api/auth/logout', token);
            const response = await app.fetch(request, env);
            expect(response.status).toBe(200);
            const result = await parseJsonResponse(response);
        ...
        it('should handle logout without token gracefully', async () => {
            const request = createTestRequest('POST', 'https://example.com/api/auth/logout');
            const response = await app.fetch(request, env);
            expect(response.status).toBe(200);
            const result = await parseJsonResponse(response);
