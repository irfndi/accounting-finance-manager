...
const vectorize = new Hono();
vectorize.use('/*', authMiddleware);
// Helper function to create vectorize service
function createVectorizeServiceInstance(vectorizeBinding, aiBinding) {
...
vectorize.post('/search', async (c) => {
    try {
        ...
        const { query, topK = 10, threshold = 0.7, filter = {}, includeChunks = false } = await c.req.json();
